...
void set_edge(int src, int dest, int w, Graph *g) // add to node edge
{
     Node* curr = NULL;    
...
int shortestPath(int src,int dest, Graph* g)
{
    ...
    struct GraphDijkstra* gr = (struct GraphDijkstra *)malloc(sizeof(*gr));
    gr->edges = mat;
    gr->vertexNum = num;
...
int Dijkstra(struct GraphDijkstra *g, int src,int dest)
{
    int V = g->vertexNum;
    int dist[V]; 
    int included[V];   
